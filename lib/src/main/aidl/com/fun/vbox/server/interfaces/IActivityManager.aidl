...
import com.fun.vbox.remote.PendingResultData;
...
interface IActivityManager{
    ...
    boolean isAppLaunchRunning(String packageName, int userId);
    ...
    int startActivity(in Intent intent,in  ActivityInfo info,in  IBinder resultTo,in  Bundle
    options, String resultWho, int requestCode, int userId,in boolean appMultiTask);
    ...
    boolean processRestarted2(String packageName, String processName, int userId);
    ...
    Bundle invokeVApp(String packageName, int userId, in Bundle param);

    int isLaunchMonopoly(String packageName);

    void setLaunchMonopoly(String packageName, int monopoly);
}