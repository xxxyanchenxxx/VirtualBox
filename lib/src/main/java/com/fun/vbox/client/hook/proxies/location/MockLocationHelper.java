...
import mirror.vbox.location.GpsStatus;
import mirror.vbox.location.GpsStatusL;
...
public class MockLocationHelper {
    ...
    public static void fakeGpsStatusN(android.location.LocationManager locationManager) {
        ...
        Object mGpsStatusListeners = LocationManager.mGpsStatusListeners.get(locationManager);
        if (mGpsStatusListeners instanceof HashMap) {
            HashMap map = (HashMap) mGpsStatusListeners;
            for(Object listenerTransport : map.values()){
                invokeSvStatusChanged(listenerTransport);
                break;
            }
        } else if (mGpsStatusListeners instanceof ArrayMap) {
            ArrayMap map = (ArrayMap) mGpsStatusListeners;
            for(Object listenerTransport : map.values()){
                invokeSvStatusChanged(listenerTransport);
                break;
            }
