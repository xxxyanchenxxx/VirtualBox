...
import com.zb.vv.client.stub.WindowPreviewActivity;
...
import mirror.vbox.content.ContentProviderNative;
...
public class VActivityManager {
    ...
    public int startActivity(Intent intent, ActivityInfo info, IBinder resultTo, Bundle options,
        ...
        return startActivity(intent, info, resultTo, options, resultWho, requestCode, userId, true);
    }

    public int startActivity(Intent intent, ActivityInfo info, IBinder resultTo, Bundle options,
                             String resultWho, int requestCode, int userId, boolean appMultiTask) {
        if (info == null) {
            info = VCore.get().resolveActivityInfo(intent, userId);
            if (info == null) {
                return ActivityManagerCompat.START_INTENT_NOT_RESOLVED;
            }
        }
        try {
            return getService().startActivity(intent, info, resultTo, options, resultWho,
                    requestCode, userId, appMultiTask);
    ...
    public int startActivity(Intent intent, int userId, boolean appMultiTask) {
        if (userId < 0) {
            return ActivityManagerCompat.START_NOT_CURRENT_USER_ACTIVITY;
        }
        ActivityInfo info = VCore.get().resolveActivityInfo(intent, userId);
        if (info == null) {
            return ActivityManagerCompat.START_INTENT_NOT_RESOLVED;
        }
        return startActivity(intent, info, null, null, null, 0, userId, appMultiTask);
    }

    public int startActivity(Intent intent, int userId) {
        return startActivity(intent, userId, true);
    }
    ...
    public boolean processRestarted(String packageName, String processName, int userId) {
        try {
            return getService().processRestarted2(packageName, processName, userId);
        } catch (RemoteException e) {
            return VirtualRuntime.crash(e, false);
        }
    }
    ...
    public Activity findActivityByToken(IBinder token) {
        Object r = ActivityThread.mActivities.get(VCore.mainThread()).get(token);
        if (r != null) {
    ...
    public void finishActivity(IBinder token) {
        ...
        while (true) {
            ...
            Activity parent = mirror.vbox.app.Activity.mParent.get(activity);
            if (parent == null) {
                break;
            }
            activity = parent;
        ...
        // because it may be overwritten.
        int resultCode = mirror.vbox.app.Activity.mResultCode.get(activity);
        Intent resultData = mirror.vbox.app.Activity.mResultData.get(activity);
        ActivityManagerCompat.finishActivity(token, resultCode, resultData);
        mirror.vbox.app.Activity.mFinished.set(activity, true);
    }
    ...
    public boolean isAppLaunchRunning(String packageName, int userId) {
        try {
            return getService().isAppLaunchRunning(packageName, userId);
        } catch (RemoteException e) {
            return VirtualRuntime.crash(e, false);
        }
    }
    ...
    public void sendBroadcast(Intent intent, int userId) {
        ...
        if (newIntent != null) {
            VCore.get().getContext().sendBroadcast(newIntent);
        }
    }
    ...
    public boolean launchApp(final int userId, String packageName, boolean preview) {
        if (VCore.get().isRun64BitProcess(packageName)) {
        ...
        if (!preview || VActivityManager.get().isAppRunning(info.packageName, userId, true)) {
            ...
            intent.putExtra("_VBOX_|no_animation", true);
    ...
    public int checkPermission(String permission, int pid, int uid) {
        try {
            return getService().checkPermission(VCore.get().is64BitEngine(), permission, pid, uid);
        } catch (RemoteException e) {
            return VirtualRuntime.crash(e, 0);
        }
    }
    ...
    public Bundle invokeVApp(String packageName, int userId, Bundle param) {
        try {
            return getService().invokeVApp(packageName, userId, param);
        } catch (Exception e) {
            return VirtualRuntime.crash(e, null);
        }
    }

    public int isLaunchMonopoly(String packageName) {
        try {
            return getService().isLaunchMonopoly(packageName);
        } catch (Exception e) {
            return VirtualRuntime.crash(e, 0);
        }
    }

    public void setLaunchMonopoly(String packageName, int monopoly) {
        try {
            getService().setLaunchMonopoly(packageName, monopoly);
        } catch (Exception e) {
            VirtualRuntime.crash(e, null);
        }
    }
}
