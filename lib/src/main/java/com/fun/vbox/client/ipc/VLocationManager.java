...
import com.fun.vbox.client.hook.proxies.location.MockLocationHelper;
...
public class VLocationManager {
    ...
    private VLocationManager() {
        LocationManager locationManager = (LocationManager) VCore.get().getContext().getSystemService(Context.LOCATION_SERVICE);
...
//                GpsStatusGenerate.fakeGpsStatus(transport);
    ...
    private boolean notifyLocation(final Object ListenerTransport, final Location location, boolean post) {
        ...
        if (!post) {
            try {
                mirror.vbox.location.LocationManager.ListenerTransport.onLocationChanged
                        .call(ListenerTransport, location);
                return true;
            } catch (Throwable e) {
                e.printStackTrace();
            }
