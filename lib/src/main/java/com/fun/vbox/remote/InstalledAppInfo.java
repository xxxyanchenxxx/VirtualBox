...
public final class InstalledAppInfo implements Parcelable {
    ...
    public String getApkPath() {
        return getApkPath(VCore.get().is64BitEngine());
    }
    ...
    public File getOdexFile() {
        return getOdexFile(VCore.get().is64BitEngine());
    }
    ...
    public int[] getInstalledUsers() {
        return VCore.get().getPackageInstalledUsers(packageName);
    }

    public boolean isLaunched(int userId) {
        return VCore.get().isPackageLaunched(userId, packageName);
    }
