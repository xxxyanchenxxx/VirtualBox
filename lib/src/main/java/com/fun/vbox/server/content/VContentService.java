...
import com.fun.vbox.server.interfaces.IContentService;
...
public final class VContentService extends IContentService.Stub {
    ...
    public void sync(SyncRequest request) {
        Bundle extras = mirror.vbox.content.SyncRequest.mExtras.get(request);
        long flextime = mirror.vbox.content.SyncRequest.mSyncFlexTimeSecs.get(request);
        long runAtTime = mirror.vbox.content.SyncRequest.mSyncRunTimeSecs.get(request);
        int userId = VUserHandle.getCallingUserId();
        int uId = VBinder.getCallingUid();
        ...
        try {
            ...
            if (syncManager != null) {
                ...
                final Account account = mirror.vbox.content.SyncRequest.mAccountToSync.get(request);
                final String provider = mirror.vbox.content.SyncRequest.mAuthority.get(request);
                if (mirror.vbox.content.SyncRequest.mIsPeriodic.get(request)) {
                    if (runAtTime < 60) {
                    ...
                    mirror.vbox.content.PeriodicSync.flexTime.set(syncToAdd, flextime);
        ...
        try {
            ...
            PeriodicSync syncToAdd =
                    new PeriodicSync(account, authority, extras,
                            pollFrequency);
            mirror.vbox.content.PeriodicSync.flexTime.set(syncToAdd, SyncStorageEngine.calculateDefaultFlexTime(pollFrequency));
            getSyncManager().getSyncStorageEngine().addPeriodicSync(syncToAdd, userId);
        } finally {
            restoreCallingIdentity(identityToken);
        }
