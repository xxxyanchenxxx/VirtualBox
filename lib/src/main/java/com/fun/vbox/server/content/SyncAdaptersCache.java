...
import com.fun.vbox.server.accounts.RegisteredServicesParser;
...
import mirror.vbox.content.SyncAdapterTypeN;
...
public class SyncAdaptersCache {
    ...
    private SyncAdapterType parseSyncAdapterType(Resources res, AttributeSet set) {
        ...
        try {
            ...
            type = mirror.vbox.content.SyncAdapterType.ctor.newInstance(contentAuthority, accountType, userVisible, supportsUploading, isAlwaysSyncable, allowParallelSyncs, settingsActivity);
            obtainAttributes.recycle();
            return type;
        } catch (Throwable e) {
            e.printStackTrace();
