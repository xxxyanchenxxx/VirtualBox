...
import com.fun.vbox.helper.compat.ContentResolverCompat;
import com.fun.vbox.helper.utils.ArrayUtils;
import com.fun.vbox.helper.utils.AtomicFile;
import com.fun.vbox.helper.utils.FastXmlSerializer;
import com.fun.vbox.os.VEnvironment;
import com.fun.vbox.server.accounts.AccountAndUser;
    ...
    public static class AuthorityInfo {
        ...
        AuthorityInfo(AuthorityInfo toCopy) {
            ...
            for (PeriodicSync sync : toCopy.periodicSyncs) {
                ...
                periodicSyncs.add(mirror.vbox.content.PeriodicSync.clone(sync));
            }
        }
        ...
        AuthorityInfo(Account account, int userId, String authority, int ident) {
            ...
            mirror.vbox.content.PeriodicSync.flexTime.set(sync, flexTime);
        ...
        AuthorityInfo(ComponentName cname, int userId, int ident) {
            ...
            final PeriodicSync periodicSync =
                    new PeriodicSync(account, authority,
                            new Bundle(),
                            DEFAULT_POLL_FREQUENCY_SECONDS);
            mirror.vbox.content.PeriodicSync.flexTime.set(periodicSync, calculateDefaultFlexTime(DEFAULT_POLL_FREQUENCY_SECONDS));
            ...
            try {
                ...
                if (add) {
                    ...
                    for (int i = 0, N = authority.periodicSyncs.size(); i < N; i++) {
                        ...
                        if (mirror.vbox.content.PeriodicSync.syncExtrasEquals(
                                toUpdate.extras,
                                syncInfo.extras)) {
                            if (toUpdate.period == syncInfo.period &&
                                    mirror.vbox.content.PeriodicSync.flexTime.get(toUpdate) == mirror.vbox.content.PeriodicSync.flexTime.get(syncInfo))
                            {
                                // Absolutely the same.
                                return;
                            }
                            authority.periodicSyncs.set(i, mirror.vbox.content.PeriodicSync.clone(toUpdate));
                            alreadyPresent = true;
                            break;
                        }
                    }
                    ...
                    if (!alreadyPresent) {
                        authority.periodicSyncs.add(mirror.vbox.content.PeriodicSync.clone(toUpdate));
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (mirror.vbox.content.PeriodicSync.syncExtrasEquals(syncInfo.extras, toUpdate.extras)) {
                            iterator.remove();
                            changed = true;
        ...
        synchronized (mAuthorities) {
            ...
            if (authority != null) {
                for (PeriodicSync item : authority.periodicSyncs) {
                    ...
                    syncs.add(mirror.vbox.content.PeriodicSync.clone(item));
                }
            }
        }
        return syncs;
    ...
    private PeriodicSync parsePeriodicSync(XmlPullParser parser, AuthorityInfo authority) {
        ...
        final PeriodicSync periodicSync =
                new PeriodicSync(authority.account, authority.authority, extras,
                        period);
        mirror.vbox.content.PeriodicSync.flexTime.set(periodicSync, flextime);
        ...
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                for (PeriodicSync periodicSync : authority.periodicSyncs) {
                    ...
                    long flexTime = mirror.vbox.content.PeriodicSync.flexTime.get(periodicSync);
