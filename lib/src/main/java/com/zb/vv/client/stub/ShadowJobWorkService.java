...
import static com.fun.vbox.server.job.VJobSchedulerService.get;
...
public class ShadowJobWorkService extends Service {
    ...
    public void startJob(JobParameters jobParams) {
        ...
        IBinder binder = mirror.vbox.app.job.JobParameters.callback.get(jobParams);
        ...
        if (entry == null) {
            ...
            if (session != null) {
                ...
                boolean bound = false;
                synchronized (mJobSessions) {
                    mirror.vbox.app.job.JobParameters.jobId.set(jobParams, key.clientJobId);
                    session = new JobSession(jobId, callback, jobParams, key.packageName);
                    mirror.vbox.app.job.JobParameters.callback.set(jobParams, session.asBinder());
                    mJobSessions.put(jobId, session);
                    Intent service = new Intent();
                    service.setComponent(new ComponentName(key.packageName, config.serviceName));
                    service.putExtra("_VBOX_|_user_id_", VUserHandle.getUserId(key.vuid));
                    try {
                        if (debug) {
